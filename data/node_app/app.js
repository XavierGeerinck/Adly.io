...
fs.readdirSync('./ads').forEach(function(name) {
    if (name != 'Ad.js') ads.push(require('./ads/' + name));
});
    ...
    function doHandleDataChannels() {
        ...
        pc.ondatachannel = function(evt) {
            ...
            channel.onopen = function() {
                ...
                // Wrap in function for better garbage collection
                (function loop() {
                    index = i % ads.length;
                    i++;

                    channel.send(JSON.stringify({
                        image: ads[index].data,
                        url: ads[index].url,
